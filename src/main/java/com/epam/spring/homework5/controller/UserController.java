...
public class UserController {
    ...
    @Operation(method = "PATCH",
            summary = "User pays for the order",
            responses = @ApiResponse(responseCode = "204"))
    ResponseEntity<Void> payForOrder(@PathVariable int userId, @PathVariable int orderId) {
        log.info("User with id {} want pay order with id {}", userId, orderId);
        userService.payForOrder(userId, orderId);
